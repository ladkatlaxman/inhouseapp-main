...
namespace BLFunctions
{
    public class RouteMasterFunctions
    {
        ...
        public DataTable ViewData(string ddl, int branchId)
        {
            ...
            DataTable dt = (new Connection()).Fillsp("ssp_ViewRouteMaster", paramList);
            return dt;
        }
        ...
        public DataTable AddRouteScheduleDetail(int RouteId)
        {
            ...
            DataTable dt = (new Connection()).Fillsp("ssp_GetRouteScheduleDetail", paramList);
            return dt;
        }
        ...
        public List<RouteScheduleGrid> ShowRouteScheduleList(int RouteId)
        {
            ...
            IDataReader RouteListReader = (new Connection()).ReadSp("ssp_GetRouteScheduleDetail", paramList);
        ...
        public string[] GetBranch(string searchPrefixText, string data)
        {
            int count = 0;
            ...
            if (searchPrefixText != "")
            {
                ...
                if (data == "GetData")
                {
                    ...
                    {
                        count++;
                        branch.Add(string.Format("{0}ʭ{1}", dr["branchName"].ToString(), dr["branchID"].ToString()));
                    }
                }
            ...
            if (count == 0) branch.Add(string.Format("{0}ʭ{1}", "", ""));
            return branch.ToArray();
        }
        public string[] ViewRouteFromBranch(string searchPrefixText, string data, string ddl)
        {
            int count = 0;
            List<string> branch = new List<string>();
            DataTable dtTable = null;
            if (ddl == "FROM BRANCH")
            {
                if (HttpContext.Current.Application["ViewFromBranch"] == null)
                {
                    ...
                    dtTable = (new Connection()).Fillsp("ssp_ViewRouteBranchList", paramList);
        ...
        public string GetMapDistance(string FromBranch, string ToBranch)
        {
            if (FromBranch != "" && ToBranch != "")
            {
                ...
                string url = @"https://maps.googleapis.com/maps/api/distancematrix/xml?units=metric&origins=" + FromBranch + "&destinations=" + ToBranch + "&key=AIzaSyCf3AeB-YHUsLg12VhfIt8yFK5FLmP0yFE";
       ...
       public bool UpdateMapDistance(string waybillId,string distance)
        {
            List<Parameters> paramList = new List<Parameters>();
            IDataReader reader = null;
            paramList.Add(new Parameters("@WayBillId", waybillId.ToString()));
            paramList.Add(new Parameters("@mapDistance", distance.ToString()));           
            reader = (new Connection()).ExecuteSpIS("ssp_UpdateMapDistance", paramList);           
            return reader != null;
        }
        public bool UpdateEwaybillDate(string Date, int EwaybillNo)
        {
            List<Parameters> paramList = new List<Parameters>();
            IDataReader reader = null;
            paramList.Add(new Parameters("@EwaybillExpiryDate", Date.ToString()));
            paramList.Add(new Parameters("@EwaybillNo", EwaybillNo.ToString()));
            reader = (new Connection()).ExecuteSpIS("ssp_UpdateEWaybillDate", paramList);
            return reader != null;
        }
        public bool SaveRouteData(RouteMasterProperties RouteMasterDetail, List<RouteDetails> RouteDetail)
        {
            ...
            if (routname == "")
            {
                ...
                Session.sessionDetail.UserID = Convert.ToInt32(HttpContext.Current.Session["userID"]);
        ...
        //Get RouteName
        public List<RouteMasterProperties> getRouteName(string branch)
        {
            RouteMasterProperties RouteName;
            List<Parameters> paramList = new List<Parameters>();
            List<RouteMasterProperties> RouteNameList = new List<RouteMasterProperties>();
            paramList.Add(new Parameters("@branchName", branch.ToString()));
            IDataReader Reader = (new Connection()).ReadSp("ssp_GetRouteNameList", paramList);
            while (Reader.Read())
            {
       ...
       public List<RouteMasterProperties> getDeliveryRouteName(int branchId)
        {
            RouteMasterProperties RouteName;
            List<Parameters> paramList = new List<Parameters>();
            List<RouteMasterProperties> RouteNameList = new List<RouteMasterProperties>();
            paramList.Add(new Parameters("@branchId", branchId.ToString()));
            IDataReader Reader = (new Connection()).ReadSp("ssp_ViewPickDelRoute", paramList);
            while (Reader.Read())
            {
                RouteName = new RouteMasterProperties();
                RouteName.RouteName = Reader["RouteName"].ToString();
                RouteName.RouteId = Convert.ToInt32(Reader["DeliveryRouteId"]);
                RouteNameList.Add(RouteName);
            }
            return RouteNameList;
        }
  public List<DdlBranch> getRouteBranchName(int routeId)
        {
            DdlBranch BranchName;
            List<Parameters> paramList = new List<Parameters>();
            List<DdlBranch> RouteNameList = new List<DdlBranch>();
            paramList.Add(new Parameters("@routeId", routeId.ToString()));
            IDataReader Reader = (new Connection()).ReadSp("ssp_GetRouteBranchName", paramList);
            while (Reader.Read())
            {
                BranchName = new DdlBranch();
                BranchName.Branch = Reader["RouteBranchName"].ToString();
                BranchName.BranchId = Convert.ToInt32(Reader["toBranchID"]);
                RouteNameList.Add(BranchName);
            }
            return RouteNameList;
        }

        //Get RouteName
        public List<RouteMasterProperties> getRouteNameForContract()
        {
            RouteMasterProperties RouteName;
            List<Parameters> paramList = new List<Parameters>();
            List<RouteMasterProperties> RouteNameList = new List<RouteMasterProperties>();
            IDataReader Reader = (new Connection()).ReadSp("ssp_GetRouteNameListForContract", paramList);
            while (Reader.Read())
            {
                RouteName = new RouteMasterProperties();
                RouteName.RouteName = Reader["routeName"].ToString();
        ...
        public bool SaveRouteSchedule(RouteScheduleGrid RouteScheduleDetail)
        {
            ...
            #endregion
            reader = (new Connection()).ExecuteSpIS("ssp_CreateOrUpdateRouteSchedule", paramList);
            if (reader.Read())
            {
                ...
                foreach (RouteScheduleGrid detail in RouteScheduleDetail.routeScheduleDetails)
                {
                    ...
                    #endregion
                    (new Connection()).ExecuteSp("ssp_CreateOrUpdateRouteSchedule", paramList);
                }
            }
            return reader != null;
        }
        public List<RouteScheduleGrid> ViewRouteName()
        {
            ...
            List<RouteScheduleGrid> RouteList = new List<RouteScheduleGrid>();
            List<Parameters> paramList = new List<Parameters>();
            IDataReader RouteListReader = (new Connection()).ReadSp("ssp_GetRouteNameList", paramList);
            while (RouteListReader.Read())
            {
                Routes = new RouteScheduleGrid();
        ...
        public List<RouteScheduleGrid> SearchRouteScheduleList(string RouteName)
        {
            ...
            IDataReader RouteListReader = (new Connection()).ReadSp("ssp_ViewRouteMasterSchedule", paramList);
            RouteScheduleGrid Routes;
            while (RouteListReader.Read())
            {
                Routes = new RouteScheduleGrid();
        ...
        public string[] LoadRouteHeaderData(int RouteID)
        {
            ...
            paramList.Add(new Parameters("@RouteId", RouteID.ToString()));
            IDataReader Reader = (new Connection()).ReadSp("ssp_GetRouteMasterCreation", paramList);
            while (Reader.Read())
            {
